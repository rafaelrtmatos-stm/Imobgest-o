...
import { LotMapExplorer } from './components/LotMapExplorer';
...
export default function App() {
  ...
  const [settingsSubTab, setSettingsSubTab] = useState<'empresa' | 'modelos'>('empresa');
  ...
  const handleSaveSale = (
    ...
    // Atualiza status do lote no mapa correspondente
  ...
  const handleDeleteSale = (saleId: string) => {
    ...
    if (confirm(`Deseja realmente excluir a venda ${saleToDelete.codigoVenda} (${saleToDelete.property.empreendimento} - Quadra ${saleToDelete.property.quadra}, Lote ${saleToDelete.property.lote})?`)) {
      ...
      // Libera o lote novamente no mapa
  ...
  // CRUD Empreendimentos & Corretores
  ...
  const handleAddLote = (empId: string, quadraNumero: string, newLote: LoteData) => {
    setEmpreendimentos(prev => prev.map(e => {
      if (e.id === empId) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans selection:bg-emerald-500 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 pb-24 lg:pb-8">
        ...
        {/* 1. EMPREENDIMENTOS & MAPAS */}
        {(activeTab === 'empreendimentos' || activeTab === 'map_explorer') && (
          <LotMapExplorer
            empreendimentos={empreendimentos}
            selectedEmpreendimentoId={selectedMapEmpreendimentoId}
            onSelectEmpreendimento={setSelectedMapEmpreendimentoId}
            onSelectLotForSale={handleSelectLotForSale}
            onUpdateCoordinates={handleUpdateCoordinates}
            onNavigateToManager={() => setActiveTab('empreendimentos')}
          />
        )}
        ...
        {/* 7. CONFIGURAÇÕES (EMPRESA & MODELOS DE DOCUMENTOS WORD) */}
