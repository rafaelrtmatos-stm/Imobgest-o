...
export const DigitalSignatureFlowModal: React.FC<DigitalSignatureFlowModalProps> = ({
  ...
  const [codeValidityHours, setCodeValidityHours] = useState<number>(72);
  ...
  // A validade (em horas) é escolhida pelo usuário no seletor abaixo (ex: 0.5h = 30 min).
  const handleGenerateCode = () => {
    if (!contract || !secondParty) return;
    setIsGeneratingCode(true);
    try {
      const res = generateSignatureCode(contract.id, secondParty.tokenAssinatura, codeValidityHours);
      onContractUpdated(res.contract);
    } finally {
        ...
        <div className="bg-white rounded-2xl w-full max-w-xl border-2 border-slate-200 shadow-2xl overflow-hidden flex flex-col">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              {/* SELETOR DE VALIDADE DO TOKEN MANUAL */}
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      title="Gerar novo código com a validade selecionada (invalida o anterior)"
                  ...
                  {secondParty.codigoValidoAte && (
                    <p className="text-[11px] text-slate-500 flex items-center space-x-1">
                      <Clock className="w-3 h-3" />
                      <span>Válido até {new Date(secondParty.codigoValidoAte).toLocaleString('pt-BR')}</span>
                    </p>
                  )}
                </div>
              ) : (
